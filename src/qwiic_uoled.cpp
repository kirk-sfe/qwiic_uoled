...
Page buffer 64 x 48 divided by 8 = 384 uint8_ts
...
static uint8_t screenmemory[] = {
	/* LCD Memory organised in 64 horizontal pixel and 6 rows of uint8_t
	 B  B .............B  -----
	 y  y .............y        \
	 t  t .............t         \
	 e  e .............e          \
...
*/
MicroOLED::MicroOLED(uint8_t deviceAddress)
{

	moled_i2c_address = deviceAddress; // Flag that the I2C address is undefined
}
...
/** \brief Send the display a command uint8_t

    Send a command via SPI, I2C or parallel	to SSD1306 controller.
	For SPI we set the DC and CS pins here, and call spiTransfer(uint8_t)
...
void MicroOLED::command(uint8_t c)
{
	...
	qwiic.writeRegister(moled_i2c_address, I2C_COMMAND, c);

}

/** \brief Send the display a data uint8_t

    Send a data uint8_t via SPI, I2C or parallel to SSD1306 controller.
	For SPI we set the DC and CS pins here, and call spiTransfer(uint8_t)
	to send the data. For I2C and Parallel we use the write functions
	defined in hardware.cpp to send the data.
*/
void MicroOLED::data(uint8_t c)
{
	...
	qwiic.writeRegister(moled_i2c_address, I2C_DATA, c);	
...
void MicroOLED::clear(uint8_t mode)
{
	...
	if (mode == ALL)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			qwiic.writeRegisterRegion(moled_i2c_address, I2C_DATA, zeros, sizeof(zeros) );
		}
	}
	else
	{
...
void MicroOLED::display(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		setPageAddress(i);
		setColumnAddress(0);
		qwiic.writeRegisterRegion(moled_i2c_address, I2C_DATA, (uint8_t *)&screenmemory[i * 0x40], 0x40 );					
...
*/
// kdb hack
void MicroOLED::print(const char * str){

	for(int i=0; i < strlen(str); i++)
		write(str[i]);

}
void MicroOLED::print(int number){
...
/** \brief Get LCD height.
    ...
    The height of the LCD return as uint8_t.
*/
...
/** \brief Get LCD width.
    ...
    The width of the LCD return as uint8_t.
*/
...
/** \brief Get font width.
    ...
    The cucrrent font's width return as uint8_t.
*/
...
/** \brief Get font height.
    ...
    The current font's height return as uint8_t.
*/
...
uint8_t MicroOLED::setFontType(uint8_t type)
{
	...
	fontMapWidth = (fontsPointer[fontType][4] * 100) + fontsPointer[fontType][5]; // two uint8_ts values into integer 16
	return true;
}
...
To use, create uint8_t array that is 64x48 pixels (384 uint8_ts). Then call .drawBitmap and pass it the array.
*/
...
//Otherwise the bitmap bitmap_converter will compress some of the uint8_ts together
void MicroOLED::drawIcon(uint8_t offsetX, uint8_t offsetY, uint8_t iconWidth, uint8_t iconHeight, uint8_t *bitArray, uint8_t arraySizeInuint8_ts, bool overwrite)
{
	...
	for (uint8_t i = 0; i < arraySizeInuint8_ts; i++)
	{
		uint16_t uint8_tNumber = rowNumber * 64 + columnNumber;

		//If we have an offset, then straddle the uint8_ts between rows
		if (bitOffset)
		{
			...
			if (overwrite == true)
			{
				screenmemory[uint8_tNumber] &= ~(0xFF << bitOffset);
				if (uint8_tNumber + 64 < (LCDHEIGHT * LCDWIDTH / 8)) //Prevent writes outside screenmemory
					screenmemory[uint8_tNumber + 64] &= ~(0xFF >> (8 - bitOffset));
			}

			//Write in new data across two rows
			screenmemory[uint8_tNumber] |= bitArray[i] << bitOffset;
			if (uint8_tNumber + 64 < (LCDHEIGHT * LCDWIDTH / 8)) //Prevent writes outside screenmemory
				screenmemory[uint8_tNumber + 64] |= bitArray[i] >> (8 - bitOffset);
		}
		else //No shift needed
		{
			if (uint8_tNumber < (LCDHEIGHT * LCDWIDTH) / 8) //Prevent writes outside screenmemory
			{
				//Regular clear
				if (overwrite == true)
					screenmemory[uint8_tNumber] = 0;
				screenmemory[uint8_tNumber] |= bitArray[i];
			}
		}
