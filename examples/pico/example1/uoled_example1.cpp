...
void shapeExample()
{
  ...
  int ball_Y = 13; //random(1 + ball_rad, oled.getLCDHeight() - ball_rad); //paddle0_Y + ball_rad;
...
void textExamples()
{
  ...
  for (int i = 0; i < 25; i++)
  {
    ...
    oled.print(33); // Print a0 reading
...
int main() {
  ...
  printf("Starting OLED Test\n\n");

    // Init the system 

    if(!oled.begin()){
    	printf("Error starting the qwiic oled device. Aboarting\n");
  		return 0;
    }
